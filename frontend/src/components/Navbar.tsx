...
const Navbar: React.FC = () => {
  ...
  // No need for manual loading since Clerk handles this
  ...
  const navLinks = [
    ...
    { name: 'Prescriptions', path: '/prescriptions', icon: <Shield className="w-5 h-5" /> },
    { name: 'Find Doctors', path: '/doctors', icon: <Search className="w-5 h-5" /> },
  ];

  const adminLinks = [
    { name: 'Admin Panel', path: '/admin', icon: <Shield className="w-5 h-5" /> },
  ];

  return (
    <nav
      className="fixed w-full z-50 bg-gradient-to-r from-blue-600 via-blue-500 to-cyan-500 shadow-lg transition-all duration-300"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          ...
          <div className="hidden md:flex md:items-center md:space-x-4">
            ...
            {/* Admin Links - Show for logged in users */}
            ...
            {!isLoaded ? (
              ...
              <div className="ml-4 flex items-center">
                <Link
                  to="/profile"
                  className="px-3 py-2 rounded-md text-sm font-medium flex items-center space-x-1 text-white hover:bg-white/20"
                >
                ...
                <button
                  ...
                  className="ml-2 px-3 py-2 rounded-md text-sm font-medium flex items-center space-x-1 text-white hover:bg-white/20"
                >
      ...
      >
        <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-gradient-to-r from-blue-600 via-blue-500 to-cyan-500 shadow-lg">
          {navLinks.map((link) => (
            <Link
              key={link.path}
              to={link.path}
          ...
          {!isLoaded ? (
            ...
            <>
              <Link
                to="/profile"
                onClick={closeMenu}
                className="block px-3 py-2 rounded-md text-base font-medium flex items-center space-x-2 text-white hover:bg-white/20"
              >
                <User className="w-5 h-5" />
                <span>Profile</span>
              </Link>
              <button
                onClick={handleLogout}
                className="w-full text-left block px-3 py-2 rounded-md text-base font-medium flex items-center space-x-2 text-white hover:bg-white/20"
              >
                <LogOut className="w-5 h-5" />
                <span>Logout</span>
              </button>
